...
export default class WorkerWrapper {
	...
	public async handleIpc(message: IWorkerMessage) {
		...
		// Check if the message is valid, and then
		if (message.command && message.command === 'init') {
			try {
				await this.init();
				self.postMessage({ id: message.id, command: 'init' });
			} catch (err) {
				return self.postMessage({ id: message.id, error: (err as Error).message });
			}
		}

		// Otherwise, not valid, return.
